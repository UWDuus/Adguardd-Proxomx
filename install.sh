...
read -p ' Name on Container datastore: ' datastore
...
pct create $number local:vztmpl/alpine-3.14-default_20210623_amd64.tar.xz --ostype alpine --hostname $name --net0 name=eth0,ip=$ip,gw=$gw,bridge=$bridge --memory 512 --cores $cpu --unprivileged 1 --cmode shell --onboot 1 --storage $datastore
pct start $number
